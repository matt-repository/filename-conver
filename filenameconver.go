...
func main()  {
	for isContinue {
		...
		fmt.Printf(" \n")
		fmt.Println(">>>>>>>>>>>>>>>>>>>>>>>>>菜单Start<<<<<<<<<<<<<<<<<<<<<<<<")
		fmt.Println("1、请输入需要转换后缀文件的文件夹绝对路径：")
		fmt.Println("---------------------------------------------------此时为：",path)
		fmt.Println("2、请输入需要转换的后缀:")
		fmt.Println("---------------------------------------------------此时为：",oldSuffix)
		fmt.Println("3、请输入需要转换成的后缀：")
		fmt.Println("---------------------------------------------------此时为：",newSuffix)
		fmt.Println("4、是否子文件夹中也全部转换,请答true或false")
		fmt.Println("---------------------------------------------------此时为：",subFolderTrans)
		fmt.Println("5、开始转换")
		fmt.Println("6、关闭程序")
		fmt.Println(">>>>>>>>>>>>>>>>>>>>>>>>>菜单End<<<<<<<<<<<<<<<<<<<<<<<<<<")
		fmt.Printf(" \n")
